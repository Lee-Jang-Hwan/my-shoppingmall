...
2. **Supabase 클라이언트 파일들**:

   **공식 Supabase 클라이언트** (`utils/supabase/` - Supabase 공식 문서 기준):
   - `server.ts`: Server Component/Server Action용 (`createClient()`)
     - `@supabase/ssr` 패키지 사용
     - 쿠키 기반 세션 관리 (표준 Supabase 인증)
     - Supabase 자체 인증 사용 시 권장
   
   - `client.ts`: Client Component용 (`createClient()`)
     - `@supabase/ssr` 패키지 사용
     - 브라우저 쿠키 기반 세션 관리
     - Supabase 자체 인증 사용 시 권장

   - `middleware.ts`: Middleware 세션 관리 유틸리티 (`updateSession()`)
     - 쿠키 기반 세션 자동 갱신
     - Middleware에서 호출

   **Clerk 통합 클라이언트** (`lib/supabase/` - Clerk + Supabase 네이티브 통합):
   ...
   - `client.ts`: 인증 불필요한 공개 데이터용 (`supabase`)
     - anon key만 사용
     - RLS 정책이 `to anon`인 데이터만 접근

   **사용 가이드**:
   - Clerk 인증 사용: `lib/supabase/clerk-client.ts` 또는 `lib/supabase/server.ts` 사용
   - Supabase 자체 인증 사용: `utils/supabase/client.ts` 또는 `utils/supabase/server.ts` 사용
   - 공개 데이터 접근: `lib/supabase/client.ts` 사용
